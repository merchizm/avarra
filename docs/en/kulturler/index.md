---
layout: page
title: Faiths & Cultures
outline: false
---

<main class="culture-hall">
  <header class="culture-hero">
    <div class="culture-hero-copy">
      <p class="lore-kicker">Avarra Encyclopaedia · Living Traditions</p>
      <h1>Faiths<br><em>and Cultures</em></h1>
      <p>In Avarra, miracles are real; whether they rise from gods, the living world, ancestors, or belief itself remains uncertain. This hall is the threshold to the rituals, laws, and daily lives born from that uncertainty.</p>
      <a class="chronicle-button" href="./dinler/">Enter the Faith Archive</a>
    </div>
    <div class="culture-orb" aria-hidden="true"><span>✦</span><small>THREE GREAT TRADITIONS</small></div>
  </header>

  <section class="culture-ledger" aria-label="Culture inventory"><div><b>3</b><span>faith families</span></div><div><b>10</b><span>sect records</span></div><div><b>4</b><span>death and return rites</span></div><div><b>∞</b><span>local spirits and customs</span></div></section>

  <section class="culture-section">
    <div class="culture-section-heading"><p class="lore-kicker">Halls of Faith</p><h2>Three great ways</h2><p>Each speaks a different language of the sacred. Choose one, then enter its archive for sects, disputes, and rites.</p></div>
    <div class="faith-gates">
      <a href="./dinler/canli-butun" class="faith-gate living"><span>◉</span><small>Living world</small><h3>Living Whole</h3><p>A faith family in which mountain is bone, river is vein, and forest is memory.</p><b>Root, stone, flow, hearth →</b></a>
      <a href="./dinler/goksel-tac" class="faith-gate celestial"><span>✧</span><small>Nine offices</small><h3>Celestial Crown</h3><p>The divine order of stars, seasons, and the first flame.</p><b>Star, crown, flame →</b></a>
      <a href="./dinler/esik-ve-atalar" class="faith-gate threshold"><span>◐</span><small>Living and dead</small><h3>Threshold & Ancestors</h3><p>Traditions in which pressing the boundary, rather than speaking to the dead, is dangerous.</p><b>Mask, veil, table →</b></a>
    </div>
  </section>

  <section class="culture-crossroads">
    <div class="crossroads-copy"><p class="lore-kicker">Threshold Records</p><h2>Death, name, and return</h2><p>The soul is understood as <b>Breath</b>, <b>Name</b>, and <b>Trace</b>. What a rite calls, whom it returns, and what price it demands changes the law of every city.</p><a href="./ruh-ogretisi">Read the teaching of the soul →</a></div>
    <div class="crossroads-links"><a href="./esik-araligi"><span>Ⅰ</span><div><small>When passage does not complete</small><strong>Threshold Interval</strong></div></a><a href="./diriltme-ayinleri"><span>Ⅱ</span><div><small>The cost of return</small><strong>Resurrection Rites</strong></div></a><a href="./muhur-nizami"><span>Ⅲ</span><div><small>Faith and power</small><strong>Seal Order</strong></div></a></div>
  </section>

  <section class="culture-section culture-section--quiet">
    <div class="culture-section-heading"><p class="lore-kicker">Atlas of Daily Life</p><h2>Traces of culture</h2></div>
    <div class="culture-trails"><article><span>❋</span><h3>Seasonal peoples</h3><p>Winter preserves, Summer shares, Autumn learns to let go, and Spring forgives debts.</p></article><article><span>≋</span><h3>Water law</h3><p>Source, tributary, delta, and floodplain are parts of one political body.</p></article><article><span>↠</span><h3>Migration and passage</h3><p>On the Endless Road, passage-right rather than property is a sacred responsibility.</p></article><article><span>◇</span><h3>Stone and labour</h3><p>Hearth families, assemblies, and tunnel guilds treat labour as the measure of citizenship.</p></article><article><span>☾</span><h3>Memory cultures</h3><p>Crystal, spore, and dream can each be how a city remembers.</p></article></div>
  </section>

  <section class="culture-crossroads">
    <div class="crossroads-copy"><p class="lore-kicker">Living Records of the Peoples</p><h2>Lineage, culture, and citizenship</h2><p>A people’s body, the law they belong to, the tradition they learn, and their class position are not the same thing. This distinction explains why faith and city life change from place to place rather than from people to people.</p><a href="./halklar-kulturleri">Read the peoples and cultures guide →</a></div>
    <div class="crossroads-links"><a href="../irklar/insanlar"><span>Ⅰ</span><div><small>Homeland, exile, and law</small><strong>Human and Veyran cultures</strong></div></a><a href="../irklar/eladrin"><span>Ⅱ</span><div><small>Season, oath, and passage</small><strong>Fey peoples</strong></div></a><a href="../irklar/cuceler"><span>Ⅲ</span><div><small>Hearth, labour, and memory</small><strong>Peoples of stone and depth</strong></div></a></div>
  </section>
</main>
